...
/// <summary>
/// 加权列表随机选择器，用于按权重从列表中随机选择元素
/// </summary>
/// <typeparam name="T">元素类型</typeparam>
public class WeightListRandom<T>
{
	private readonly List<WeightedEntry> _entries = new();
	private double _totalWeight;
	private readonly Random _random = new();
	...
	/// <param name="weight">权重</param>
	/// <returns>this</returns>
	public WeightListRandom<T> Add(T element, double weight)
	{
		if (weight < 0)
			throw new ArgumentException("Weight must be non-negative", nameof(weight));

		_entries.Add(new WeightedEntry(element, weight));
		_totalWeight += weight;
		return this;
	}
	...
	/// <param name="weightFunc">权重计算函数</param>
	/// <returns>this</returns>
	public WeightListRandom<T> AddRange(IEnumerable<T> elements, Func<T, double> weightFunc)
	{
		foreach (var element in elements)
			Add(element, weightFunc(element));
		return this;
	}
	...
	private class WeightedEntry
	{
		...
		public WeightedEntry(T element, double weight)
		{
			Element = element;
			Weight = weight;
		}
