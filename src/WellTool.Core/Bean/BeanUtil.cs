...
public static class BeanUtil
{
	...
	public static string GetFieldName(string methodName)
	{
		...
		// 处理getter方法
		if (methodName.StartsWith("get", StringComparison.OrdinalIgnoreCase) && methodName.Length > 3)
		{
			return methodName.Substring(3, 1).ToLower() + methodName.Substring(4);
		}

		// 处理setter方法
		if (methodName.StartsWith("set", StringComparison.OrdinalIgnoreCase) && methodName.Length > 3)
		{
			return methodName.Substring(3, 1).ToLower() + methodName.Substring(4);
		}

		// 处理is方法（布尔类型）
		if (methodName.StartsWith("is", StringComparison.OrdinalIgnoreCase) && methodName.Length > 2)
		{
			return methodName.Substring(2, 1).ToLower() + methodName.Substring(3);
		}
