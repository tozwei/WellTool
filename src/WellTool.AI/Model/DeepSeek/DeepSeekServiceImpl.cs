...
namespace WellTool.AI.Model.DeepSeek
{
    ...
    /// DeepSeek服务实现类
    /// </summary>
    public class DeepSeekServiceImpl : BaseAIService, DeepSeekService
    {
        /// <summary>
        /// 构造
        /// </summary>
        /// <param name="config">AI配置</param>
        public DeepSeekServiceImpl(AIConfig config) : base(config) { }
        ...
        {
            return BetaAsync(prompt).GetAwaiter().GetResult();
        }
        ...
        /// <param name="prompt">题词</param>
        /// <param name="callback">流式数据回调函数</param>
        public void Beta(string prompt, System.Action<string> callback)
        {
            BetaStreamAsync(prompt, callback).GetAwaiter().GetResult();
        }
        ...
        private async Task<string> ChatAsync(List<Message> messages)
        {
            ...
            var response = await SendPostAsync("/v1/chat/completions", json);
            return response;
        }
        ...
        private async Task ChatStreamAsync(List<Message> messages, System.Action<string> callback)
        {
            ...
            var json = JsonSerializer.Serialize(requestBody);
            await SendPostStreamAsync("/v1/chat/completions", json, callback);
        }
        ...
        /// <param name="prompt">题词</param>
        /// <returns>AI的回答</returns>
        private async Task<string> BetaAsync(string prompt)
        {
            var requestBody = new
            {
                model = config.GetModel(),
            ...
            var json = JsonSerializer.Serialize(requestBody);
            var response = await SendPostAsync("/v1/beta/completions", json);
            return response;
        }
        ...
        /// <param name="prompt">题词</param>
        /// <param name="callback">回调函数</param>
        private async Task BetaStreamAsync(string prompt, System.Action<string> callback)
        {
            var requestBody = new
            {
                model = config.GetModel(),
            ...
            var json = JsonSerializer.Serialize(requestBody);
            await SendPostStreamAsync("/v1/beta/completions", json, callback);
        }
        ...
        {
            return await SendGetAsync("/v1/models");
        }
        ...
        {
            return await SendGetAsync("/v1/balance");
        }
    }
}
