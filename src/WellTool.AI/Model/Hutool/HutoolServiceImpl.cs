...
namespace WellTool.AI.Model.Hutool
{
    ...
    /// Hutool服务实现类
    /// </summary>
    public class HutoolServiceImpl : BaseAIService, HutoolService
    {
        /// <summary>
        /// 构造
        /// </summary>
        /// <param name="config">AI配置</param>
        public HutoolServiceImpl(AIConfig config) : base(config) { }
        ...
        private async Task<string> ChatAsync(List<Message> messages)
        {
            ...
            var response = await SendPostAsync("/chat", json);
            return response;
        }
        ...
        private async Task ChatStreamAsync(List<Message> messages, System.Action<string> callback)
        {
            ...
            var json = JsonSerializer.Serialize(requestBody);
            await SendPostStreamAsync("/chat/stream", json, callback);
        }
        ...
        {
            return await SendGetAsync("/balance");
        }
    }
}
