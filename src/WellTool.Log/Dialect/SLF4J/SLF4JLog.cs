...
public class SLF4JLog : AbstractLog
{
    ...
    /// <param name="logger">SLF4J 日志对象</param>
    public SLF4JLog(string name, object logger) : base(name)
    {
        _logger = logger;
    }
    ...
    /// </summary>
    /// <param name="logger">SLF4J 日志对象</param>
    public SLF4JLog(object logger) : this("SLF4J", logger)
    {
    }
    ...
    public override bool IsEnabled(WellTool.Log.Level.Level level)
    {
        try
        {
            var methodName = "is" + level.ToString() + "Enabled";
            var method = _logger.GetType().GetMethod(methodName, Type.EmptyTypes);
            if (method != null)
            {
                return (bool)method.Invoke(_logger, null);
            }
        }
        catch
        {
    ...
    public override void Log(WellTool.Log.Level.Level level, Exception t, string format, params object[] arguments)
    {
        try
        {
            ...
            {
                var method = _logger.GetType().GetMethod(methodName, new[] { typeof(string), typeof(object[]) });
                if (method != null)
                {
                    method.Invoke(_logger, new object[] { message, arguments });
                }
                else
                {
                    var method2 = _logger.GetType().GetMethod(methodName, new[] { typeof(string) });
                    if (method2 != null)
                    {
                        method2.Invoke(_logger, new object[] { message });
                    }
                }
            }
        }
