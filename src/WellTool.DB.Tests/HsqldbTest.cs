...
public class HsqldbTest
{
    ...
    {
        // 测试 HSQLDB 数据库连接
        var connectionString = "jdbc:hsqldb:mem:testdb;shutdown=true";
        var dataSource = new TestDataSource(connectionString, "HSQLDBDriver");
    ...
    /// </summary>
    [Fact]
    public void TestHsqldbTransaction()
    {
        // 测试 HSQLDB 数据库的事务操作
        var connectionString = "jdbc:hsqldb:mem:testdb;shutdown=true";
        var dataSource = new TestDataSource(connectionString, "HSQLDBDriver");
    ...
    /// </summary>
    [Fact]
    public void TestHsqldbSpecialFeatures()
    {
        // 测试 HSQLDB 数据库的特殊功能
        var connectionString = "jdbc:hsqldb:mem:testdb;shutdown=true";
        var dataSource = new TestDataSource(connectionString, "HSQLDBDriver");
        ...
        // 测试 HSQLDB 特有的功能，如内存表
        var sql = "CREATE MEMORY TABLE memory_table (id INT PRIMARY KEY, value VARCHAR(100))";
        var result = db.Execute(sql);
        Assert.Equal(0, result);
